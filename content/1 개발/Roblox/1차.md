...
```lua
-- server leader board
local platform = script.Parent -- 이 스크립트의 부모 파트를 platform 변수에 저장한다

local function fade() -- 발판을 점점 사라지게 만드는 함수
    for count = 1, 10 do -- count를 1부터 10까지 반복한다
        platform.Transparency = count / 10 -- 투명도를 0.1씩 증가시킨다
        wait(0.1) -- 0.1초 기다린다
    end

    platform.CanCollide = false -- 발판의 충돌을 꺼서 플레이어가 통과하게 만든다
    wait(5) -- 5초 동안 사라진 상태로 기다린다

    platform.CanCollide = true -- 발판의 충돌을 다시 켠다
    platform.Transparency = 0 -- 발판을 다시 완전히 보이게 만든다
end

platform.Touched:Connect(fade) -- 발판에 무언가 닿으면 fade 함수를 실행한다
